...
for ki, k_out in enumerate(a_points):
    ...
    try:
        ...
        print(f"k_out = {k_out}, k_cyc/k_out = {k_cyc / k_out}, ratio = {nu_cyc / nu_lin}")
        ratios[gi, ki] = nu_cyc / nu_lin
    except RuntimeError:
        p_eq = None
        p_eq_res = None
        nu_cyc = np.nan
...
np.savetxt(f"{fi}.dat", ratios)
fig, ax = plt.subplots(figsize=(14,12))
xx, yy = np.meshgrid(a_points, g_points / gamma_E)
...
ax.set_xlabel(r'$ k_{out} $ ')
fig.colorbar(cs, ticks=levs)
fig.tight_layout()
plt.savefig(f"{fi}.pdf")
